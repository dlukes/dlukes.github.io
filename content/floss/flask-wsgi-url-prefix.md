...
merits and drawbacks of splitting up technical information in a way
...
The fix: just use a fully-featured production WSGI server, e.g.
[Gunicorn]. I promise `SCRIPT_NAME` works there.
...
Just to confirm Flask's builtin development server doesn't properly
handle `SCRIPT_NAME` -- run the app with it:
